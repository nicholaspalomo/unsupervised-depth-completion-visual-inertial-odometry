...
class Camera:
    ...
    def init_camera_model(self, bag_file, robot_name, camera_name):
        ...
        with rosbag.Bag(bag_file) as bag:
            info_topic = "/" + robot_name + "/" + camera_name + "/fisheye1/camera_info"
...
class LiDARCameraRosbagData:
    def __init__(self, robot_name, camera_name):
        ...
        self.camera_image_topic = '/' + robot_name + '/' + camera_name + '/fisheye1/image_raw_throttle'
        # /telemax1/camera_t265_front/fisheye1/image_raw_throttle
        self.camera_depth_image_topic = '/' + robot_name + '/' + camera_name + '/aligned_depth_to_color/image_raw_throttle/compressedDepth'
        self.camera_info_topic = '/' + robot_name + '/' + camera_name + '/fisheye1/camera_info' # '/husky1/camera_front/color/camera_info'
        ...
        for vision_bag_name in self.filenames["vision"]:
            with rosbag.Bag(vision_bag_name, "r") as bag:
                ...
                for topic, msg, t in bag.read_messages(topics=self.camera_info_topic):
                    ...
                    rgb_image_dataset = image_group.create_dataset("image", data=np.empty((0, h*w), dtype=np.uint8), compression="gzip", chunks=True, maxshape=(None,h*w))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--robot", help="robot name, e.g. husky1", required=False, type=str, default="telemax1")
    parser.add_argument("--camera", choices=["camera_t265_front", "camera_left", "camera_right"], help="name of camera", required=False, type=str, default="camera_t265_front")
