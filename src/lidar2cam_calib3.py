# Align realsense depth image and sparse point cloud
...
def main(args):
    ...
    with h5py.File(args.data_file, 'r') as h5:
        ...
        depth_image = h5['image']['depth_image'][INDEX, :]
    ...
    depth_image = np.reshape(depth_image, (h, w))
    depth_image = depth_image / np.max(depth_image)

    if debug:
        plt.subplot(121),plt.imshow(depth_image),plt.title('Realsense Depth Image')
        plt.xticks([]), plt.yticks([])
        plt.subplot(122),plt.imshow(depth_map_interp),plt.title('Depth Map (Interp)')
        plt.xticks([]), plt.yticks([])
        plt.show()
    ...
    grad_x_image, grad_y_image, grad_mag_image, dir_image = compute_gradients(validity_map, depth_image, neighborhood=3, thresh=0.4)
    ...
    n = 4
    rot = np.pi * 0.04 * np.linspace(-1.0, 1.0, num=n+1)
    tran = 0.2 * np.linspace(-1.0, 1.0, num=n+1)
    ...
    for z in tran:
        ...
        for p in rot:
            ...
            R_cam2velo = rpy2rotmat(r, p, yaw)
            roll, pitch, yaw_ = rotmat2rpy(R_cam2velo.T)

            rN, pN, yawN = r0 + roll, p0 + pitch, yaw0 + yaw_
            ...
            descriptors_depth, descriptors_image = get_descriptors(depth_map, depth_image, dir_depth, dir_image, depth_map=depth_map, image=gray, num_descriptors=10, neighborhood=25)
    ...
    print("Original params: r, p, y: {}, {}, {}. x, y, z: {}, {}, {}".format(r0, p0, yaw0, tf_velo2cam[0,-1], tf_velo2cam[1,-1], tf_velo2cam[2,-1]))
    print("Optimal params: r, p, y: {}, {}, {}. x, y, z: {}, {}, {}".format(r, p, yaw, x, y, z))
